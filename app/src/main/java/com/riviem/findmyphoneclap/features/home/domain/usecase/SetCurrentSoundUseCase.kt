...
class SetCurrentSoundUseCase @Inject constructor(
    private val settingsRepository: SettingsRepository,
    private val audioClassificationService: AudioClassificationService
) {
    suspend fun execute(currentSoundId: Int) {
        settingsRepository.setCurrentSoundId(soundId = currentSoundId)
        audioClassificationService.setCurrentSoundId(soundId = currentSoundId)
    }
}